...
switch(lower(classifier))
	...
	%	if nargin==4, cPrm=gmmcTrain(DS, trainPrm, showPlot); end
		...
		if length(gmmcOpt.arch.gaussianNum)==1
			gmmcOpt.arch.gaussianNum=gmmcOpt.arch.gaussianNum*ones(classNum, 1);
		end
		...
		%	fprintf('Training GMM for class %d...\n', i);
			...
			theGmmOpt=gmmcOpt;
			theGmmOpt.arch.gaussianNum=gmmcOpt.arch.gaussianNum(i);
	...
	case 'lsec'
		...
		cPrm.coef=A\b;
		logLike=[];
		recogRate=1-sum((A*cPrm.coef-b).^2)/sum((b-mean(b)).^2);		% This is actually R-square for linear regression
		hitIndex=[];
	otherwise
		error(sprintf('Unknown classifier: %s (in %s)', classifier, mfilename));
end
...
switch(lower(classifier))
	case {'qc', 'nbc', 'gmmc', 'knnc', 'svmc', 'lsec'}

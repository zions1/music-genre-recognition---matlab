function [fn, fp]=detPlot(data1, data2, opt)
...
%		data1=randn(1,300)-1;
%		data2=randn(1,300)+1;
%		opt=detPlot('defaultOpt');
%		subplot(1,2,1);
%		opt.scaleMode='linear';
%		detPlot(data1, data2, opt);
%		subplot(1,2,2);
%		opt.scaleMode='log';
%		opt.textRotation=270;
%		detPlot(data1, data2, opt);
%
%	See also detGet.
%
...
if ischar(data1) && strcmpi(data1, 'defaultOpt')	% Set the default options
	fn.scaleMode='linear';
	fn.textRotation=30;
	return
end
...
set(gca, 'xscale', opt.scaleMode, 'yscale', opt.scaleMode);
xlabel('False-positive rate (%)');
ylabel('False-negative rate (%)');
title(sprintf('DET curve (%s mode)', opt.scaleMode)); 
...
for i=1:length(id);
	...
	h=text(fps(index)*100, fns(index)*100, sprintf('  \\theta=%.2f (fp=%.2f%%, fn=%.2f%%)', th, 100*fps(index), 100*fns(index)));
	set(h, 'rotation', opt.textRotation);
end
axis square, grid on

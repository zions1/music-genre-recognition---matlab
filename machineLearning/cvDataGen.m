...
for i=1:opt.foldNum
	...
	if isfield(DS, 'inputName'), cvData(i).TS.inputName=DS.inputName; end
	if isfield(DS, 'outputName'), cvData(i).TS.outputName=DS.outputName; end
	cvData(i).VS.input=DS.input(:, vIndex);
	cvData(i).VS.output=DS.output(:, vIndex);
	if isfield(DS, 'inputName'), cvData(i).VS.inputName=DS.inputName; end
	if isfield(DS, 'outputName'), cvData(i).VS.outputName=DS.outputName; end
end

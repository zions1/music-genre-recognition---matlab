...
set(contourH, 'linewidth', 2);
axis(axisLimit); axis square;
pointH = zeros(dataNum,1);
textH = zeros(dataNum,1);
for i = 1:dataNum,
%	pointH(i) = line(x(i), y(i), 'color', 'g', 'linestyle', ':', 'markersize', 10, 'erase', 'xor');
	pointH(i) = line(x(i), y(i), 'color', 'g', 'linestyle', ':', 'markersize', 10);
%	textH(i) = text(x(i), y(i), num2str(i), 'fontsize', 8, 'visible', 'off', 'erase', 'xor');
	textH(i) = text(x(i), y(i), num2str(i), 'fontsize', 8, 'visible', 'off');
end

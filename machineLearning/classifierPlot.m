...
if dim~=2 && strcmpi(mode, lower('2dPdf')) , error('%s is only for 2d dataset!', mfilename); end
pointNum=51;
...
if strcmpi(classifier, 'nbc') && strcmpi(mode, lower('1dPdf'))
	...
	for i=1:classNum
		for j=1:dim
			...
			ylabel(strPurify(sprintf('class %d (%s)', i, DS.outputName{i})));
		end
	end
	surfObj=[];
	return
...
switch(lower(mode))
	case lower('2dPdf')		
		for i=1:classNum
			...
			axis([-inf inf -inf inf -inf inf]); box on;
			% ====== Contour plot

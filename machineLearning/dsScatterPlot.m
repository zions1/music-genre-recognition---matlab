...
%	circleH=line(nan, nan, 'marker', 'o', 'color', 'k', 'erase', 'xor');
	circleH=line(nan, nan, 'marker', 'o', 'color', 'k');
%	textH=text(0, 0, '', 'hori', 'center', 'vertical', 'top', 'erase', 'xor');
	textH=text(0, 0, '', 'hori', 'center', 'vertical', 'top');
...
if isfield(DS, 'hitIndex')	% Display the mis-classified data points
	missIndex=1:dataCount;
	...
	titleString = sprintf('%d misclasified points denoted by "x".', length(missIndex));
	title(titleString);
%	h=findobj(gca, 'type', 'line'); set(h, 'markersize', 10);
end

function [th, fp, fn, gPrm1, gPrm2, a, b, ths, fps, fns]=detGet(data1, data2, opt, showPlot);
...
if ischar(data1) && strcmpi(data1, 'defaultOpt')	% Set the default options
	th.prior=[];	% Use data count by default
	return
end
if nargin<3||isempty(opt), opt=feval(mfilename, 'defaultOpt'); end
if nargin<4, showPlot=0; end

if isempty(opt.prior), opt.prior=[length(data1), length(data2)]; end	% Data counts as priors

n1=opt.prior(1); n2=opt.prior(2);
allData=[data1(:); data2(:)]';

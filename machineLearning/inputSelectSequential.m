...
if nargin<4, param=feval([classifier, 'Train'], 'defaultOpt'); end
...
modelNum=inputNum*(2*dim-inputNum+1)/2;		% No. of models

% ====== Construct classifiers with different input variables
if plotOpt
	fprintf('\nConstruct %d %s models, each with up to %d inputs selected from %d candidates...\n', modelNum, classifier, inputNum, dim);
end
selectedInput=[];
modelIndex=1;
t0=clock;

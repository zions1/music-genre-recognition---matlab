...
if ischar(data) && strcmpi(data, 'defaultOpt')
	...
	gmmModel.arch.gaussianNum=2;
	gmmModel.arch.covType=2;
...
%if isfield(opt.arch, 'gaussianNum')
%	gaussianNum = opt.arch.gaussianNum;
%else	% opt.arch is in fact the gmmModel.
%	gmmModel = opt.arch;
...
if isfield(opt.arch, 'initModel')
	gmmModel=opt.arch.initModel;
else

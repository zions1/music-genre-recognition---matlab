...
gaussianNum=gmmcOpt.arch.gaussianNum;	% This should be a matrix of C by K, where C is the no. of classes and K is the no. of trials
...
for j=1:trialNum
	...
	%	fprintf(' class %d... ', i);
		...
		try
			theGmmSpec=gmmcOpt;
			theGmmSpec.arch.gaussianNum=gaussianNum(i,j);
...
validTrialIndex=length(gmmcOpt.arch.gaussianNum);
...
if plotOpt
	...
	legend({'Inside test', 'Outside test'}, 'location', 'southeast');

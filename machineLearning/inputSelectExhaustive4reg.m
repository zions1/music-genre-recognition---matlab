function [bestSelectedInput, bestRecogRate, allSelectedInput, allRecogRate, elapsedTime] = inputSelectExhaustive(DS, inputNum, plotOpt)
% inputSelectExhaustive: Input selection via exhaustive search for regression
%	Usage:
%		bestSelectedInput=inputSelectExhaustive(DS)
%		bestSelectedInput=inputSelectExhaustive(DS, inputNum)
%		bestSelectedInput=inputSelectExhaustive(DS, inputNum, plotOpt)
...
%	allRecogRate(i) = feval(classifier, DS2, param);
	A=[DS2.input; ones(1, size(DS2.input,2))]';		% Augmented by 1's vector
	b=DS2.output';
	allRecogRate(i)=perfLoo4lse(A, b);
	if plotOpt
		fprintf('modelIndex %d/%d: selected={%s} => RMSE = %f\n', i, modelNum, join(inputName(allSelectedInput{i}), ', '), allRecogRate(i));
	end
end

[bestRecogRate, b] = min(allRecogRate);
...
if plotOpt
	inputSelectPlot(allRecogRate, allSelectedInput, inputName, mfilename);
end

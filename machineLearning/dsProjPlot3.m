		...
		if k <= plotNum,
			...
			if opt.showAxisLabel
				if isfield(DS, 'inputName')
					xlabel(['x', num2str(sequence(k,1)), ': ', strPurify(DS.inputName{sequence(k,1)})]);
					ylabel(['x', num2str(sequence(k,2)), ': ', strPurify(DS.inputName{sequence(k,2)})]);
					zlabel(['x', num2str(sequence(k,3)), ': ', strPurify(DS.inputName{sequence(k,3)})]);
				else
